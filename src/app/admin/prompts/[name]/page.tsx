...
export default function PromptDetailPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  async function fetchPromptVersions() {
    try {
      const response = await fetch(`/api/admin/prompts/${encodeURIComponent(name)}`);
  ...
  async function handleActivate(versionId: string) {
    ...
    try {
      const response = await fetch(`/api/admin/prompts/${encodeURIComponent(name)}/activate`, {
        method: "POST",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              <Link href="/admin/prompts" className="text-gray-600 hover:text-gray-900 mr-4">
                ← プロンプト一覧
              </Link>
              <h1 className="text-xl font-bold">{data.name}</h1>
              <span className="ml-4 px-2 py-1 text-xs font-semibold rounded bg-gray-200 text-gray-700">
                {data.type}
              </span>
            </div>
          </div>
          ...
          <div className="lg:col-span-2">
            {selectedVersion && (
              <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="mb-4">
                  <h3 className="text-sm font-medium text-gray-700 mb-2">プロンプト内容:</h3>
